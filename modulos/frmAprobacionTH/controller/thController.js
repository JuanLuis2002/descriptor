...
var THController = {
    ...
    cargarPendientes: function() {
        ...
        for (var i = 0; i < pendientes.length; i++) {
            ...
            html += '<tr>' +
                ...
                '<ul class="dropdown-menu dropdown-menu-end" style="z-index: 2100;"><li><button class="dropdown-item" onclick="THController.verDetalle(' + d.id + ')"><i class="fas fa-eye me-2 text-info"></i>Revisar</button></li></ul></div></td>' +
    ...
    cargarGestionados: function() {
        ...
        for (var i = 0; i < gestionados.length; i++) {
            ...
            html += '<tr>' +
                '<td><div class="dropdown"><button class="btn btn-sm btn-outline-primary dropdown-toggle" type="button" data-bs-toggle="dropdown"><i class="fas fa-ellipsis-v"></i></button>' +
                '<ul class="dropdown-menu dropdown-menu-end" style="z-index: 2100;"><li><button class="dropdown-item" onclick="THController.verHistorialGestion(' + d.id + ')"><i class="fas fa-history me-2 text-secondary"></i>Ver historial</button></li>' + reportes + '</ul></div></td>' +
                '<td><strong>' + (d.codigo || 'DES-' + d.id) + '</strong></td>' +
                '<td>' + (d.puesto || 'Sin título') + '</td>' +
